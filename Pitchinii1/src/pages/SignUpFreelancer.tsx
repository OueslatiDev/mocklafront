  ...
  import { validateEmail, validateNotEmpty, validatePassword } from "../utilFunctions/ValidateFunction (1)";
  import { send } from "../utilFunctions/sendData";
  import Navbar from "../components/Navbar";
  import SecondHeader from "../components/SignUpSecondHeader";
  ...
  const JointFreelancerP: FunctionComponent = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const res = await fetch("http://localhost:3001/api/competence/all");
          if (!res.ok) {
          ...
          const uniqueDomains = [...new Set(domainAndSkillsData.map(item => item.domaine))];
    ...
    async function validation2() {
      ...
      if (step == "03" && !Password && !ConfirPassword) {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          send(false, fileFormData, navigating, "http://localhost:3001/api/fichier");
        }
      }
    }
    ...
    const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (files) {
        const updatedFileObjects: SelectedFile[] = Array.from(files).map(file => ({
          file,
          type: type, // Access the file type using file.type
        }));
        setSelectedFiles(prevSelectedFiles => [...prevSelectedFiles, ...updatedFileObjects]);
            ...
            <div className="self-stretch flex flex-row items-center md:items-start justify-start pt-0 pb-1.5 pr-[41px] pl-[46px] box-border max-w-full text-13xl text-blue-1 mq1050:pl-[23px] mq1050:box-border">
              ...
              <div className={step == "02" ? showenInput : hiddenInput}>
                <Select
                  ...
                  displayEmpty
                  multiple
                  renderValue={(selected: string[]) => ( // Explicitly specify the type of selected
                    <div>
                      {selected.length === 0
